...
- One **top-level folder per domain**—e.g. `escrows`, `milestones`, `funding`, `evidence`, `wallet`—not per file type at the repository root.
...
This is independent of **monorepo vs single app**: you can have `src/features` inside one package or `apps/community-dashboard/src/features` inside an app package.

### CMinds app examples

| App | Typical feature slices |
| --- | --- |
| `community-dashboard` | `escrow-create`, `milestones`, `evidence`, `release` |
| `cminds-dashboard` | `escrow-review`, `milestone-approval`, `disputes`, `admin` |
| `funding-dashboard` | `escrow-detail`, `fund`, `deposit-history` |
| `public-viewer` | `escrow-transparency`, `milestone-progress` |
...
| **`hooks/`** (or **`composables/`**) | Stateful logic tied to the UI framework: queries/mutations, form state, wallet subscriptions, browser APIs. **No** raw HTTP client usage scattered everywhere—call into **`services/`** from here.     |
| **`services/`**                      | **Side-effectful I/O**: HTTP clients to `core-api`, Trustless Work SDK wrappers, WebSocket setup. Exposes functions or a small class per use case. **No** framework-specific render API (no JSX in React, etc.).             |
...
- **Shared UI** — design system, headless primitives, icons: live in **`components/ui`**, **`packages/ui`** (`@repo/ui`), or vendor scope—not under `features/`.
- **Cross-feature contracts** — DTOs, enums, or events shared by many slices: **`src/types`**, **`packages/contracts`**, or API-generated clients—so two features do not import each other’s internals for types only.
...
4. **New multi-field form** → validation in `schemas/` (if used), orchestration in a hook, markup in `components/` (see also `FORMS.mdc`).
5. **Uncertainty** → mirror the **closest existing slice** in the same app so the repo stays predictable.

---

## Naming and legacy folders

Folder names on disk are the **source of truth** for imports. Use **kebab-case** for feature folders (`escrow-create`, `milestone-review`). Legacy or misspelled names often persist for stability; document them in onboarding rather than “fixing” names in a drive-by change.
