"use client";

import { zodResolver } from "@hookform/resolvers/zod";
import { toastError } from "@repo/ui/lib/toast";
import { useRouter, useSearchParams } from "next/navigation";
import { useEffect, useState } from "react";
import { useForm } from "react-hook-form";

import { otpSchema, type OtpFormValues } from "../schemas/otp.schema";
import { ensureTotpEnroll } from "../services/ensure-totp-enroll";
import { getSupabaseBrowserClient } from "../services/supabase-browser";
import { safeReturnTo } from "../utils/safe-return-to";
import { useSupabaseAuth } from "./useSupabaseAuth";

export type TotpEnrollResult = {
  factorId: string;
  qrCode: string;
  secret: string;
  uri: string;
};

export function useTotpEnroll() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const returnTo = safeReturnTo(searchParams.get("returnTo"));
  const { refreshAuthState, session } = useSupabaseAuth();
  const userId = session?.user.id ?? null;
  const [enroll, setEnroll] = useState<TotpEnrollResult | null>(null);
  const [loading, setLoading] = useState(false);
  const [enrolling, setEnrolling] = useState(true);

  const form = useForm<OtpFormValues>({
    resolver: zodResolver(otpSchema),
    defaultValues: { code: "" },
  });

  useEffect(() => {
    if (!userId) {
      return;
    }

    let cancelled = false;

    async function startEnroll(): Promise<void> {
      setEnrolling(true);
      try {
        const supabase = getSupabaseBrowserClient();
        const payload = await ensureTotpEnroll(supabase, userId!);

        if (cancelled) {
          return;
        }

        setEnroll(payload);
      } catch (err) {
        if (cancelled) {
          return;
        }

        if (err instanceof Error && err.message === "TOTP_ALREADY_VERIFIED") {
          router.replace(
            `/mfa/challenge?returnTo=${encodeURIComponent(returnTo)}`,
          );
          return;
        }

        toastError(
          "Authenticator Setup Failed",
          err instanceof Error
            ? err.message
            : "Unable to start TOTP enrollment. Please try again.",
        );
      } finally {
        if (!cancelled) {
          setEnrolling(false);
        }
      }
    }

    void startEnroll();

    return () => {
      cancelled = true;
    };
  }, [userId, returnTo, router]);

  async function onSubmit(values: OtpFormValues): Promise<void> {
    if (!enroll) {
      return;
    }

    setLoading(true);

    try {
      const supabase = getSupabaseBrowserClient();
      const { error: verifyError } = await supabase.auth.mfa.challengeAndVerify(
        {
          factorId: enroll.factorId,
          code: values.code,
        },
      );

      if (verifyError) {
        toastError(
          "Verification Failed",
          verifyError.message ||
            "Invalid authenticator code. Please try again.",
        );
        return;
      }

      await supabase.auth.refreshSession();
      await refreshAuthState();
      router.replace(returnTo);
    } catch (err) {
      toastError(
        "Verification Failed",
        err instanceof Error
          ? err.message
          : "Invalid authenticator code. Please try again.",
      );
    } finally {
      setLoading(false);
    }
  }

  return { form, onSubmit, loading, enrolling, enroll };
}
