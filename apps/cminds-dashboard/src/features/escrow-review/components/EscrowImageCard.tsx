...
import type { EnrichedOperatorEscrow } from "../types";

type EscrowImageCardProps = {
  item: EnrichedOperatorEscrow;
  className?: string;
  style?: CSSProperties;
};
...
export const EscrowImageCard = ({
  ...
  const actionCount = pendingReviewCount + disputedCount;

  return (
    <Link
      href={`/dashboard/escrows/${metadata.escrow_id}`}
      style={style}
      className={cn(
        ...
        <dl className="mt-auto grid grid-cols-2 gap-x-4 gap-y-3">
          ...
          <div className="min-w-0">
            ...
            <dd className="mt-1 truncate text-sm font-medium text-foreground">
              {formatCreatedDate(metadata.created_at)}
            </dd>
          </div>
