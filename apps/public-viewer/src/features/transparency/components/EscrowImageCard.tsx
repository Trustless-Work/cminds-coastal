...
export const EscrowImageCard = ({
  ...
  const fallback =
    FALLBACK_COVERS[coverIndex % FALLBACK_COVERS.length] ?? FALLBACK_COVERS[0];
  ...
  return (
    <Link
      href={`/escrows/${encodeURIComponent(escrow.escrow_id)}`}
      style={style}
      className={cn(
        "group flex h-full min-w-0 flex-col overflow-hidden rounded-[24px] border border-border bg-background p-3 transition-transform duration-[250ms] ease-out hover:-translate-y-1 sm:p-4",
        className,
      )}
    >
