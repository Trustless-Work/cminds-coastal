...
/** ISO-3166 alpha-2 → regional-indicator flag emoji (no image assets needed). */
...
/** Sorted alphabetically by country name. Dial codes include the leading "+". */
...
/** Sensible default for this pilot (coastal Costa Rica). */
export const DEFAULT_COUNTRY: Country =
  COUNTRIES.find((country) => country.iso2 === "CR") ?? COUNTRIES[0]!;

const DIAL_CODE_MATCHERS = [...COUNTRIES].sort(
...
/** Split a stored E.164-ish value ("+50688887777") into country + local digits. */
export function parsePhoneNumber(value: string | null | undefined): ParsedPhone {
  ...
  const country =
    DIAL_CODE_MATCHERS.find((entry) => digits.startsWith(entry.dialCode)) ??
    null;

  if (!country) {
    return { country: null, national: digits.replace(/\D/g, "") };
  }
...
/** Compose a stored value from a country + local digits. Empty local → "". */
export function composePhoneNumber(country: Country, national: string): string {
